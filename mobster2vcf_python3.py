...
__copyright__ = """
Copyright (C) 2015 Fritjof Lammers
    ...
    with open(out_fname, mode='w') as writer:
        ...
        _INFO = [OrderedDict(
            [("ID", "SVTYPE"), ("Number", "1"), ("Type", "String"), ("Description", "\"Type of structural variant\"")]),
                 ...
                 OrderedDict([("ID", "MEINFO"), ("Number", "4"), ("Type", "Integer"),
                              ("Description",
        ...
        for key, value in _metadata.items():
            writer.write("##{}={}\n".format(key, value))
        for elem in _INFO:
            _INFO_line = ["%s=%s" % (key, value) for key, value in elem.items()]
...
def _format_vcffile(dictreader, vcffile):
    ...
    for line in dictreader:
        ...
        SVLEN = "NA"  # int(line['border3']) - int(line['border5'])
        SVTYPE = "INS"  # line['Mobile Element'].strip()
        MEITYPE = line['Mobile Element'].strip()
        SCORE = "NA"
        QUAL = "."  #int(line['.'])

        INFO ='PROGRAM=mobster;SVTYPE={};SVLEN={};SCORE={};MEINFO={},{},{},{};DP={}'.format(SVTYPE,SVLEN,SCORE,MEITYPE,IMPRECISE_START,IMPRECISE_END,"NA",DP)
